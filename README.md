...
- `src/culinary/store.cljc` — `Store` protocol +
  `MemStore`: registered order tickets, committed records, an
  append-only audit ledger.
- `src/culinary/advisor.cljc` — `Advisor` protocol;
...
- `src/culinary/actor.cljc` — `build-graph`, `run-request!`,
