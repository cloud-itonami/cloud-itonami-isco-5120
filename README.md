...
- `src/culinary/store.kotoba` — `Store` protocol +
  `MemStore`: registered order tickets, committed records, an
  append-only audit ledger.
- `src/culinary/advisor.kotoba` — `Advisor` protocol;
...
- `src/culinary/actor.kotoba` — `build-graph`, `run-request!`,
