(ns culinary.actor
  "CulinaryActor — the ISCO-08 5120 independent culinary actor as a
  `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one cook-support/plating operation
  ...
  The unconditional invariant: the CulinaryAdvisor can never directly
  commit a record or dispatch a robot action the CulinaryGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [culinary.advisor :as advisor]
            [culinary.governor :as governor]
            [culinary.store :as store]))

(defn build-graph
  "Build a compiled CulinaryActor graph. `store` implements
  `culinary.store/Store`. `advisor` implements
  `culinary.advisor/Advisor` (defaults to `mock-advisor`).
